...
const Search = () => {
    ...
    const projectsFiltered= searching==='' ? [] : projects.filter(element=>element.name.toLowerCase().includes(searching.toLocaleLowerCase()))
    
    return (
        <Transition.Root show={ search  } as={Fragment} afterLeave={ ()=>setSearching('')}>
            <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto mt-20 p-4 sm:p-20 md:p-20" onClose={handleSearch}>
                ...
                <Combobox
                    ...
                    onChange={(project)=>(window.location= `/project/${project._id}`)}
                >
